...
The system SHALL provide a background daemon process for executing scheduled jobs and async tasks. The daemon SHALL be a separate Rust binary (`meridian-daemon`) in the workspace, using tokio runtime.

> **Build & bundling:** The daemon is registered as a Tauri sidecar via `externalBin` in `tauri.conf.json`. In development, `npm run dev` automatically compiles it (`cargo build -p meridian-daemon`) and copies it to `src-tauri/binaries/` with the target-triple suffix (via `scripts/copy-sidecar.js`). In production, Tauri bundles the sidecar into the app package. The main app spawns it using `app.shell().sidecar("meridian-daemon")` (Tauri shell plugin). Permissions are scoped in `capabilities/default.json` (`shell:allow-spawn` for the sidecar).
